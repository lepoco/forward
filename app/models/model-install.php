...
final class Model extends Models
{
	 ...
	 * @return   void
	 */
	private function setEncryptAlgo(): string
	{
		/** Password hash type */
	...
	private function buildConfig($args): void
	{
		...
		$config .= "\n\n/** Passwords hash type */\ndefine( 'FORWARD_ALGO', " . $this->setEncryptAlgo() . " );";
		...
		$config .= "\n\n/** Session salt */\ndefine( 'SESSION_SALT', '" . Crypter::salter(64) . "' );";
		$config .= "\n/** Passowrd salt */\ndefine( 'PASSWORD_SALT', '" . Crypter::salter(64) . "' );";
		$config .= "\n/** Nonce salt */\ndefine( 'NONCE_SALT', '" . Crypter::salter(64) . "' );";
	...
	private function buildTables($database, $args): void
	{
		...
		$dbFile = file(APPPATH . 'code/database/forward.sql');
		$queryLine = '';
	...
	private function fillData($database, $args)
	{
		...
		$q1 = Crypter::salter(5, 'U');
		$q2 = Crypter::salter(5, 'U');
		$q3 = Crypter::salter(5, 'U');
		$q4 = Crypter::salter(5, 'U');
		...
		$this->Forward->User->signIn(array('user_id' => 1, 'user_role' => 'admin'));
		$database->close();
	}
