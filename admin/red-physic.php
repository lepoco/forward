	...
	*/
	class RED
	{
		...
		*/
		...
		{
			$DIR_URL = parse_url(urldecode('/'.trim(str_replace(rtrim(dirname($_SERVER['SCRIPT_NAME']),'/'),'',$_SERVER['REQUEST_URI']),'/')));
			...
			if($DIR_URL[1] == '')
				define('RED_PAGE', '_forward_home');
			else if($DIR_URL[1] == RED_DASHBOARD)
				define('RED_PAGE', '_forward_dashboard');
			else
				define('RED_PAGE', filter_var($DIR_URL[1], FILTER_SANITIZE_STRING));
